...
def resolve_prices_sync(
    *,
    ...
    for i in range(num_nights):
        ...
        matching = [ov for ov in date_overrides if ov.start_date <= night <= ov.end_date]
        if matching:
            best = matching[-1]
            results.append(
                ResolvedNight(
            ...
            results.append(ResolvedNight(date=night, price=base_price, source="base", label=None))
